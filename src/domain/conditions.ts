...
// '뿐 아니라'류(뿐·뿐만 뒤에 조사 이·은·는·이는이 붙어도, 띄어쓰기 무관, '-아니고' 포함)는 배제가
// 아니라 긍정 병렬이다 — "효과 측정뿐 아니라 확대도 합시다."가 '아니' 표지에 부분 문자열로 걸려
// 참가자가 요청한 MEASURE가 사라졌다(PR #10 Codex 30차 검토 P1). 조사 '이'("뿐만이 아니라", 31차)·
// 보조사 '은'("뿐만은 아니라", 32차)이 차례로 빠져 있어 조사 자리를 한 번에 열거한다. 표지를 보기
// 전에 창에서 지운다. '뿐' 없는 "검수가 아니라 측정을 합시다"의 대조 부정은 그대로 부정이다.
const POSITIVE_PARALLEL = /뿐만?(?:이는|이|은|는)?\s?아니[라고]/g;
