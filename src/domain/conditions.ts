...
// '뿐 아니라'·'뿐만 아니라'·'뿐만이 아니라'·'뿐이 아니라'(띄어쓰기 무관, '-아니고' 포함)는 배제가
// 아니라 긍정 병렬이다 — "효과 측정뿐 아니라 확대도 합시다."가 '아니' 표지에 부분 문자열로 걸려
// 참가자가 요청한 MEASURE가 사라졌다(PR #10 Codex 30차 검토 P1). 조사 '이'가 붙은 "뿐만이
// 아니라"가 첫 패턴에 빠져 있었다(31차 P1). 표지를 보기 전에 창에서 지운다. "검수가 아니라
// 측정을 합시다"의 대조 부정('가/은/는 아니라')은 그대로 부정이다.
const POSITIVE_PARALLEL = /뿐(?:만이?|이)?\s?아니[라고]/g;
