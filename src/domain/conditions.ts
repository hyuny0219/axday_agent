...
// 초성 ㄷ+ㅚ/ㅙ 음절은 되~됳·돼~됗 범위다. 어절 시작 판정은 앞이 한글이 아닐 때에 더해 조사
// (은·는·이·가·을·를·도·만·과·와·에·로)일 때도 인정한다 — "검수는안할게요"처럼 조사와 '안'을 붙여 쓰면
// 앞 글자가 한글이라 걸리지 않았다(23차). "불안하면"·"미안하"·"보안하"는 앞 글자가 조사가 아니라
// 여전히 부정이 아니다.
const AN_NEGATION = /(?:^|[^가-힣]|[은는이가을를도만과와에로])안(?:\s|[하-힣]|[되-됳돼-됗])/;
