...
지시서 1.5와 docs/AGENT_BOARDROOM_SPEC.md의 구현 순서를 따른다.

| 단계 | 범위 | 이 계획에서의 위치 |
| --- | --- | --- |
| P0 | 안건 ② 완주. live: 임원 4명 실제 AI 판단·토론·표결, 실제 AI '내 발언 정리'·회의 요약. scripted: 규칙 엔진 데모(키 없이 실행, 리허설·장애 대안). 공통: 추천 문구·직접 입력·조건 확인·최종 투표·시간 만료·무입력 복귀·운영 메뉴·전체화면·오프라인 자산 | M0~M4·T25(scripted 완료), M-L1~M-L3(live), M5·M6, 첫 PR |
| P1 | 안건 ①·③ 역할 프롬프트·근거·③ 정규화, 관람 뷰, 꼬리표·MOTION 안건 종류 표시 | M7~M9, 두 번째 PR |
| P2 | 비용·응답시간·발언 길이 튜닝, 선택 출력·음성 확장. 실제 AI를 P2로 미루지 않는다 | 행사 전 운영값 확정 후 |

P0에서 하지 않는 것: 관람 창과 전송 프로토콜 구현, ③ 전용 정규화, 서기 모드 전용 UI, 프린터 출력, 모바일·터치 대응. 공개 payload selector와 전송 어댑터 인터페이스는 P0에서 정의한다(완료).

키 없이 완주하는 것은 scripted 모드이며 live 완료를 대신하지 않는다. live 완료는 실제 모델 평가(T32)와 mock 제공자 E2E(T30·T31)를 모두 통과했을 때다.
...
| 서버 | P0부터 Node 서버 1개(`server/`): 모델 호출·응답 검증·mock 제공자. 정적 빌드 + 서버로 운영, scripted 모드는 서버 없이 동작 | 키를 브라우저에 두지 않음. 검증은 서버가 수행(v0.8) |
...
**시간은 두 개의 시계다.** ~~240초 체험 deadline(연장 불가)과 무입력 시계(클릭·키 입력·실제 스크롤로만 갱신, RESULT 진입 시 재시작). 같은 tick에서 둘 다 만료면 무입력 복귀가 우선한다.~~ **2026-09-22(T50) 제거** — 체험은 시간으로 끝나지 않는다. 주입형 `Clock`은 서버 핸들러·orchestrator의 지연 측정(8초 상한)에만 남는다. 아래 M3 행과 시간 테스트 항목은 당시 계획 기록이다(`AGENT_BOARDROOM_SPEC.md` 6장이 현재 기준).
...
| M-L1 live 도메인·서버 | Statement·Transcript·모드·표 메타(T26 완료), 서버 골격·응답 검증·mock/Anthropic 제공자(T27), 역할 프롬프트·라운드·표결 핸들러(T28) | 단위: 검증 거절 케이스, mock 라운드 4명·timeout·invalid·주입 격리 | 2일 |
| M-L2 live 연결 | 오케스트레이터·어댑터(T29), 화면 연결·모드 표시·live E2E(T30) | mock 서버로 live 완주·UNCAST 경로·scripted 강제 경로 E2E | 2일 |
| M-L3 비서실장·평가 | 내 발언 정리·회의 요약 live(T31), 실제 모델 평가 하네스(T32) | 요청 상한·revision 폐기·실패 시 원문 유지; 키가 있으면 실측 기록, 없으면 미검증으로 보고 | 1.5일 |
...
P0 합계 참고 소요: 약 13일(1인, v0.8 live 5.5일 포함). 현장 마우스·물리 키보드 한글 입력과 실제 장비 검증은 코드로 대신할 수 없으므로 리허설 항목으로 남긴다.
...
| 단계 | 산출물 | 완료 확인 |
| --- | --- | --- |
| M10 운영값 튜닝 | 모델·effort·발언 길이·max_tokens 조정, 비용·지연 측정 반복, 선택 출력·음성 확장 | T32 하네스 결과로 행사 운영값 확정 |
...
| P0.5 | T40 | 후속 단순화: CAIO 질문 + 빠른 답, 접힌 직접 입력, 답글형 반응 | 1일 |
