...
- 만들 것:
  ...
  5. 테스트: `tests/content/aiAssistant.test.ts`에 chairBriefing 3문장·insight 4개·issues 3개·조건 미리보기 ID가 조건 목록과 같음. E2E `e2e/briefing.spec.ts`: 의장 브리핑·쟁점 3개·조건 칩 4개가 보이고 칩을 눌러도 아무 일도 없음, 진행 스트립이 BRIEFING에서 ①을 가리키고 DISCUSS에서 ③을 가리킴. 기존 E2E·스크린샷 갱신.
- 허용 경로: `src/content/`, `src/components/screens/BriefingScreen.tsx`, `src/components/parts/ProgressStrip.tsx`(신규)·`Header.tsx`, `src/app/App.tsx`(ProgressStrip 배선만), `src/styles/`, `tests/`, `e2e/`, `docs/screenshots/`.
