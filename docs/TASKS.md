...
| T54 | 진행 중 | 프롬프트의 근거 인용을 자료명으로 교체 — 공통 가드레일을 "자료 이름으로 인용, 발언 문장 속 ID 금지, evidenceIds는 ID"로 바꾸고 `<meeting_record>` 자료 목록을 이름 먼저·id 뒤로 표기, `PROMPT_VERSION` v4 → v5. `scripts/eval-set-run.ts`에 발언 속 자료 ID 검사(`findEvidenceIdMentions`, `--check` 출력)를 추가. **실측 대기** — 이 세션에 실제 키가 없어 v4 before(머지 커밋 82468c0에서 실행)와 v5 after를 아직 재지 못했다. 절차와 결과 칸은 `docs/eval/tuning-v5.md`. 단위 테스트 4건. |
...
- 크기: S.
- 진행(2026-09-25): 코드·검사·문서 틀 완료(v5). 실측은 실제 키가 있는 새 세션에서 `docs/eval/tuning-v5.md`의 절차대로 before(v4, 82468c0)·after(v5)를 돌린 뒤 표를 채운다.
