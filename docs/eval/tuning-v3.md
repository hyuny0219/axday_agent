...
- **집계를 코드로**: `scripts/eval-set-run.ts`에 `findStyleViolations()`를 심었다. 문장 단위로 쪼개고, 말미 **인용** 괄호("…입니다(E3,E4)")를 제거한 뒤 존댓말 종결 여부를 본다. 인용 판정은 괄호 안에 자료 ID(E1~)나 발언 ID(st-·op-)가 있는지로 하고, "검토합니다(권한 확인 필요)." 같은 설명성 괄호는 제거하지 않는다 — 제거하면 그 안의 금지 종결이 가려진다. 문장 경계는 문장부호 뒤 공백뿐 아니라 공백 없이 이어지는 경우("필요.검토하겠습니다.")도 나눈다 (두 규칙 모두 PR #10 Codex 2차 검토 반영. 기존 기록 재집계 결과는 187/1/0으로 동일하다). `--check <파일.jsonl>`로 모델 호출 없이 기존 기록을 재집계할 수 있고, 실행 후에도 자동으로 검사 결과를 찍는다. 존댓말 종결로 인정하는 어미는 합쇼체(-습니다/-니까/-십시오)뿐 아니라 해요체(-해요/-이에요/-예요/-죠 등)도 포함한다 — 규칙 문구가 "존댓말(-습니다/-합니다 등)"이지 격식체 한정이 아니기 때문이다(PR #10 Codex 6차 검토 반영, 재집계 결과 187/1/0 동일). 맨끝 "요" 하나만 보고 통과시키지는 않는다("…지정 필요."는 위반). -니까는 합쇼체 의문형(-습니까/-ㅂ니까 — 앞 음절 종성이 ㅂ인지 유니코드로 판정)만 인정하고 반말 연결형 "없으니까."는 위반이며, "-인데요/-은데요/-는데요"는 "데요"로 판정한다(Codex 7차 검토). 해요체 "-아요/-어요"는 축약 활용("맡겨요"·"알려요"·"둬요")까지 덮도록 앞 음절을 열거하지 않고 음절 구조(종성 없음 + 중성 ㅏ·ㅐ·ㅓ·ㅔ·ㅕ·ㅖ·ㅘ·ㅙ·ㅝ·ㅞ)로 판정한다. "필요"·"중요"·"수요"는 이 구조가 아니라 여전히 위반이다(Codex 13차 검토, 재집계 187/1/0 동일). 규칙은 `tests/scripts/eval-set-run.test.ts`로 고정했다.
