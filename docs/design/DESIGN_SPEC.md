...
나머지 이미지도 위 링크로 직접 열어 비교한다. 이 이미지들은 AI로 제작된 체험 콘셉트 목업이다. 실존 임원의 사진·실제 프로그램 캡처가 아니다. 배경·캐릭터를 재사용할 때 UI 문자가 이미지에 포함된 상태로 제품 화면에 중복 노출하지 않는다. 캐릭터 단독 이미지가 없으면 CSS 기반 역할 아바타·아이콘으로 P0를 완성하고 이후 별도 자산으로 교체 가능하게 만든다.
...
| 브리핑 | 상단 안건, 하단 2×2 근거 카드 | 근거는 펼치되 기본 텍스트만으로 이해 |
