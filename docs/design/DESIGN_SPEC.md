...
| AI 브리핑 | [briefing.png](assets/briefing.png) | 자료4장·상시 AI 정리·사전 구성 표기 — AI 정리 카드·배지는 T52(2026-09-23)에서 제거된 시각 레퍼런스 |
...
- 타이머 없음(2026-09-22 결정, T50): 240초 만료·75/90초 무입력 복귀와 그에 딸린 카운트다운·경고 문구를 모두 없앴다. 화면 어디에도 남은 시간 표시를 두지 않는다.
...
- 결과의 ‘AI가 도운 일’은 실제 사용한 AI 비서실장 도움만 나열하고, 없으면 미사용 문구를 보인다(자동 자료 정리 데모 표시는 T52 카드 제거에 맞춰 PR #10 Codex 27차에서 함께 없앴다 — 표시하지 않은 것을 기록하지 않는다). 비실사 5인 아바타는 토론과 결과에 동일한 인물 매핑을 사용한다.
- 운영 버튼 클릭, 전체화면 및 물리 키보드 한글 입력 동작은 [구현 지시서](../../CLAUDE_IMPLEMENTATION.md) 3장을 따른다. 무입력 75초 안내/90초 복귀 검수는 T50(2026-09-22)에서 타이머와 함께 제거했다. 폰트와 모든 이미지·아이콘은 로컬 번들하며, 네트워크 단절 상태로 새로고침해 검증한다.
...
- ‘운영’ 버튼을 클릭해 초기화·전체화면 메뉴를 연다. (단순 커서 이동이 무입력 90초 시계를 갱신하지 않는다는 조건은 T50에서 시계와 함께 소멸했다.)
...
- 무대 띠는 장식이다. 컨테이너에 `aria-hidden="true"`를 두고, 읽어야 할 정보는 모두 아래 본문 블록에 그대로 둔다. (무대 애니메이션을 무입력 활동으로 세지 않는다는 조건은 T50에서 무입력 시계와 함께 소멸했다.)
...
- 헤더: 유리 패널(`backdrop-filter: blur(8px)`). 4분 시계의 벽시계 pill 복제와 헤더 타이머는 T50에서 제거했다 — 무대 띠에 시간 표시를 두지 않는다.
...
- 본문 폭 축소 대응: 브리핑 자료 카드 4장은 2열로 자료명·해석·원문을 클램프 없이 상시 노출한다(T52 — 쟁점 목록 제거, 원문 클램프는 PR #10 Codex 29차에서 제거). DISCUSS 근거 카드는 1280에서 제목+한 줄 해석만 보이고 `details`로 펼친다(testid 유지). 추천 문구 6개는 2열. 결과 5석 카드는 3+2로 감싼다(1280에서도 3+2). live 판단 근거는 2줄(1280에서 1줄), 남은 우려는 1줄로 클램프한다. 임원 4열 카드(OPINIONS·DISCUSS 상단)는 2열.
...
| 반응 | 무대 + 내 발언 인용(2줄 클램프) + 빠른 답 3 ↔ 직접 답하기(**같은 자리 전환**: 열면 빠른 답 3개가 입력창·조건 칩으로 바뀌고, 닫으면 복귀) | 후속 질문(`followUp.question`, 발화자는 시나리오 `followUp.askedBy` — 안건 ② 사내 게시판 익명제에서는 CFO의 신고 처리 담당자 질문) + 임원 반응 2×2 답글 | 없음 |
