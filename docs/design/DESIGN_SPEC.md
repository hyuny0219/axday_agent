...
| 안건 선택 | [agenda-select.png](assets/agenda-select.png) | 3개 카드·선택 강조(v0.5 시각 참고, ③ 문구 적용 제외) |
...
| 결과(v0.5 시각 참고, 표결 수치 적용 제외) | [result.png](assets/result.png) | 동등한 5석·의견 반영 기록 |
...
PPT·result.png·agenda-select.png는 보존된 v0.5 콘셉트 레퍼런스다. 기존 찬성4·보류1 예시는 최신 시나리오②의 네 조건 경로에 해당하지 않는다. v0.6에서는 해당 경로가 찬성5·보류0이며 참가자 반대 시 찬성4·반대1이다. 기존 이미지의 CISO 보류를 최신 상태로 복제하지 않는다. 시나리오①도 같은 변경이다.

이번 문서 개정은 이미지/PPT 바이너리를 새로 만든 작업이 아니다. 개발·검수 화면은 최신 시나리오를 기준으로 렌더하고, 기존 PPT를 행사 자료로 다시 사용하기 전 해당 페이지·결과 이미지의 숫자와 상태를 교체해야 한다. 문서에서 이미지 링크를 소개할 때 ‘v0.5 시각 참고, 표결 수치 적용 제외’라고 표시한다.
...
안건③ 선택 문구는 ‘시범 운영이 끝났고, 확대 규모를 정해야 합니다.’, 부제는 ‘검증한 AI 서비스를 어디까지 확대할까?’다. agenda-select.png의 구제목은 구현 문구로 복제하지 않는다.
