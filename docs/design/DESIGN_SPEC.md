...
PPT와 result.png는 보존된 v0.5 콘셉트 레퍼런스다. 기존 찬성4·보류1 예시는 최신 시나리오②의 네 조건 경로에 해당하지 않는다. v0.6에서는 해당 경로가 찬성5·보류0이며 참가자 반대 시 찬성4·반대1이다. 기존 이미지의 CISO 보류를 최신 상태로 복제하지 않는다. 시나리오①도 같은 변경이다.
